...
if __name__ == "__main__":
    ...
    parser.add_argument("--extended_inference", action='store_true', help="extend test df")
    ...
    for fold, (train_idx, valid_idx) in enumerate(fold_iter):
        ...
        for site in [1, 2]:
            if opt.skip_existing and (export_dir/f'fold{fold}_site{site}.pt').exists():
                LOGGER(f'checkpoint fold{fold}_site{site}.pt already exists.')
                continue

            LOGGER(f'===== TRAINING SITE {site} FOLD {fold} =====')

            train_fold = train.iloc[train_idx].query('site_id == @site')
            valid_fold = train.iloc[valid_idx].query('site_id == @site')
            ...
            try:
                trainer = TorchTrainer(model, serial=f'fold{fold}_site{site}', device=None)
    ...
    for fold, (train_idx, valid_idx) in enumerate(fold_iter):
        ...
        for site in [1, 2]:
            ...
            if not (export_dir/f'fold{fold}_site{site}.pt').exists():
                LOGGER(f'fold{fold}_site{site}.pt missing. No target to predict.')
                continue

            LOGGER(f'===== INFERENCE SITE {site} FOLD {fold} =====')
            ...
            model = cfg.model(**cfg.model_params)
            checkpoint = torch.load(export_dir/f'fold{fold}_site{site}.pt', 'cpu')
            # clean up checkpoint
            if 'checkpoints' in checkpoint.keys():
                del checkpoint['checkpoints']
                torch.save(checkpoint, export_dir/f'fold{fold}_site{site}.pt')
            ...
            trainer = TorchTrainer(model, serial=f'fold{fold}_site{site}', device=opt.gpu)
            ...
            if opt.extended_inference:
                results = []
                for (_, pid, lat), p, t in zip(valid_data.pids, pred_logits.reshape(-1), target_fold.reshape(-1)):
                    results.append({'prediction_id': f'{pid}_{lat}', 'pred': p, 'target': t})
                results = pd.DataFrame(results).groupby('prediction_id').agg({'pred': 'mean', 'target': 'max'}).reset_index()
                pred_logits = results['pred'].values.reshape(-1, 1)
                target_fold = results['target'].values.reshape(-1, 1)

            eval_score_site, thres_site = eval_metric(torch.from_numpy(pred_logits), torch.from_numpy(target_fold))
            LOGGER(f'PFbeta site {site}: {eval_score_site:.5f} threshold: {thres_site:.5f}')
            preds.append(pred_logits)
            preds_bin.append((sigmoid(pred_logits) > thres_site).astype(float))
            targets.append(target_fold)
            thres_fold.append(thres_site)

        preds = np.concatenate(preds, axis=0)
        preds_bin = np.concatenate(preds_bin, axis=0)
        targets = np.concatenate(targets, axis=0)
        eval_score_fold = eval_metric.pfbeta(targets, preds_bin)
        LOGGER(f'PFbeta: {eval_score_fold:.5f}')
        for im, metric_f in enumerate(cfg.monitor_metrics):
            LOGGER(f'Monitor metric {im}: {metric_f(torch.from_numpy(preds), torch.from_numpy(targets)):.5f}')
