...
if __name__ == "__main__":
    ...
    '''
    outoffolds = []
    selfpreditions = np.full((1, len(train), 1), 0, dtype=np.float32)
    ...
    if not (export_dir/f'nocv.pt').exists():
        ...
        if 'checkpoints' in checkpoint.keys():
            ...
            torch.save(checkpoint, export_dir/f'nocv.pt')
        ...
        pred_logits = trainer.predict(
            valid_loader, parallel=inference_parallel, progress_bar=opt.progress_bar)
    ...
    with open(str(export_dir/'predictions_nocv.pickle'), 'wb') as f:
        pickle.dump({
            'folds': None,
